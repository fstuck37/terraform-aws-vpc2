locals {
  ...
  flow_log_iam_role_arn    = var.flow_log_traffic_type == "s3" ? null : lookup(lookup( aws_iam_role.flowlog_role, "${var.name-vars["account"]}-${replace(var.region,"-", "")}-${var.name-vars["name"]}-flow-log-role", null), "arn", null)
